...
public isolated function audit_request(workflow workflow, string username, string patientID, string sysname) returns error? {
    match workflow {
        ...
        PATIENT_DEMOGRAPHICS_UPDATE => {
            check sendAuditEvent(audit_PDU_request(username, patientID, sysname, "0"));
        }
        PATIENT_DEMOGRAPHICS_CREATE => {
            check sendAuditEvent(audit_PDC_request(username, patientID, sysname, "0"));
        }
        // ...
...
public isolated function audit_PDC_request(string username, string patientID, string sysname, string outcome) returns InternalAuditEvent {
    return {
        typeCode: "110112",
        subTypeCode: "CREATE",
        actionCode: "C",
...
public isolated function audit_PDU_request(string username, string patientID, string sysname, string outcome) returns InternalAuditEvent {
    return {
        typeCode: "110112",
        subTypeCode: "UPDATE",
        actionCode: "U",
