...
Generate a test token before running any API calls:
...
> *"The hospital's local MRN (`CR-2024-0156`) is kept alongside her NIC number. The CR assigns a national CRUID that links her across all facilities."*
...
> *"Any facility connected to the HIE can search by name, NIC, phone, or other demographics."*

---

### Scene 3 — Maria moves to Kandy, registers at Kandy Teaching Hospital

6 months later, Maria relocated. She visits Kandy Teaching Hospital for a fever.
...
**Expected:** `200 OK` — Because the NIC (`200012345678`) already exists, this is a **conditional update**, not a duplicate.

> *"The conditional PUT uses the NIC as the match key. Since Maria already exists, the CR updates her record instead of creating a duplicate. This is the ITI-104 transaction."*
...
### Scene 5 — Maria's record needs to be deactivated
...
```bash
# Confirm it appears in inactive search
curl -H "Authorization: Bearer $TOKEN" \
  "http://localhost:9090/fhir/r4/Patient?family=Silva&active=false"
```

> *"The CR uses soft-delete — the record is deactivated, not erased, preserving the audit trail and historical data."*
...
```bash
curl -X PUT "http://localhost:9090/fhir/r4/Patient?identifier=http://colombo-general.moh.lk/mr|CG-10234" \
  ...
  -d '{
    ...
    "address": [{"use": "home", "city": "Colombo", "postalCode": "00100", "country": "LK"}]
  }'
```
...
```bash
curl -X PUT "http://localhost:9090/fhir/r4/Patient?identifier=http://kandy-district.moh.lk/mr|KD-5567" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/fhir+json" \
  -d '{
    "resourceType": "Patient",
...
> *"In reality, patients visit multiple facilities. Without a shared identifier like NIC, each hospital creates its own record."*
...
**Expected:** Bundle with both Kamal Perera records ranked by score.

> *"The $match operation (ITI-119) uses weighted scoring across name, DOB, gender, phone, and postal code. It returns candidates with a confidence grade: certain (≥0.95), probable (≥0.80), or possible (≥0.60)."*
...
> *"The dedup engine uses blocking keys (Soundex of name + DOB, phone number, identifiers) to narrow candidates, avoiding O(n²) comparisons."*
...
```bash
curl -H "Authorization: Bearer $TOKEN" \
  "http://localhost:9090/fhir/r4/Patient/dedupreject?patient1={CRUID_1}&patient2={CRUID_2}"
```

> *"Rejected pairs get an exclusion code. Future dedup runs skip them automatically."*
...
```bash
curl "http://localhost:9093/audits?sortOrder=desc&limit=20"
```
