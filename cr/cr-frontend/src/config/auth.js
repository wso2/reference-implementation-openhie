...
const VALID_MODES = ['oidc', 'simulated'];

let _authConfigError = null;

if (!rawMode) {
  _authConfigError =
    'VITE_AUTH_MODE is not set. Set it to "oidc" or "simulated" in your .env file.';
} else if (!VALID_MODES.includes(rawMode)) {
...
export const authConfigError = _authConfigError;

/** 'oidc' | 'simulated' | null (null only when authConfigError is set) */
export const authMode = _authConfigError ? null : rawMode;
 ...
 */
export const groupsClaim = import.meta.env.VITE_OIDC_GROUPS_CLAIM?.trim() || 'groups';

const _metadataUrl = import.meta.env.VITE_OIDC_METADATA_URL?.trim() || undefined;
