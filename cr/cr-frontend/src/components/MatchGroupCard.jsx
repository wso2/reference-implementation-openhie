...
export default function MatchGroupCard({
  ...
  // Selection state for multi-patient groups
  const [selected, setSelected] = useState(new Set());

  // Reset selection when group changes or collapses
  useEffect(() => {
    setSelected(new Set());
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Actions — only for 2-patient groups */}
      ...
      <Collapse in={isExpanded}>
        ...
        >
          {/* Legend + selection hint for multi-patient groups */}
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
            <Box sx={{ display: 'flex', gap: 3 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                />
                <Typography
                  sx={{ fontSize: 13, fontWeight: 600, color: 'success.main' }}
                >
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                />
                <Typography
                  sx={{ fontSize: 13, fontWeight: 600, color: 'error.main' }}
                >
                  Unmatched Fields ({group.unmatchedFields.length})
                </Typography>
              </Box>
            </Box>
            {isPending && isMulti && (
          ...
          <Grid container spacing={2}>
            {group.patients.map((patient, idx) => {
              ...
              return (
                <Grid key={patient.id} size={{ xs: 12, md: 12 / patientCount }}>
                  <Card
                    variant="outlined"
                    ...
                    >
                      ...
                      <Box>
                        <Typography sx={{ fontWeight: 600, fontSize: 13 }}>
                          M{idx + 1}: {getPatientIdentifier(patient)}
                        </Typography>
                        <Typography
                          sx={{
                            fontSize: 11,
          ...
          {/* Action bar for multi-patient groups */}
          {isPending && isMulti && (
            <Box
              sx={{
                display: 'flex',
