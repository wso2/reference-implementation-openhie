...
export default function PatientMatchDialog({ open, patient, onClose }) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (!cancelled) setError(err.message || 'Match failed.');
      } finally {
  ...
  const gradeLabel = (grade) => {
    const map = { certain: 'Certain', probable: 'Probable', possible: 'Possible', 'certainly-not': 'No Match' };
              ...
              <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 2, py: 0.5 }}>
                ...
                <Box sx={{ flex: 1 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
                    ...
                    <Chip
                      label={gradeLabel(match.matchGrade)}
                      size="small"
                      sx={{
