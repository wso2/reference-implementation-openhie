...
function _key(filters: AuditLogFilters, sortOrder: string): string {
  const { subtype = null, since = null, before = null } = filters || {};
  return `${sortOrder}:${JSON.stringify({ subtype, since, before })}`;
}

function _get(key: string): CacheEntry | null {
...
function _set(key: string, logs: AuditLogEntry[], offset: number, hasMore: boolean): void {
...
export function useAuditLog() {
  ...
  const filtersRef = useRef<AuditLogFilters>({});
