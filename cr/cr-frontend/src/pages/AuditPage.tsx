          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5, mt: 1.5 }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              {entry.outcome === 'success' ? (
                <Chip icon={<CheckCircle size={14} />} label="Success" size="small"
                  sx={{ bgcolor: '#dcfce7', color: '#166534', fontWeight: 600, fontSize: '11px' }} />
              ) : (
                <Chip icon={<XCircle size={14} />} label="Failed" size="small"
        ...
        <Box sx={{ px: 3, py: 2 }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5, mt: 1.5 }}>
            ...
            {entry.reason && (
              ...
              >
                <Typography sx={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: entry.outcome === 'success' ? '#166534' : '#991b1b', mb: 0.5 }}>
                  Reason
                </Typography>
...
export default function AuditPage() {
  const { logs, loading, loadingMore, error, hasMore, sortOrder, fetchLogs, loadMore, toggleSort } = useAuditLog();
  ...
  useEffect(() => {
    ...
    if (filterAction !== 'all') filters.subtype = filterAction;
    if (dateFrom) filters.since = dateFrom.toISOString();
    if (dateTo) filters.before = dateTo.toISOString();
    fetchLogs(filters);
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
      ...
      >
        <IconButton
          onClick={toggleSort}
          size="small"
          sx={{ bgcolor: 'background.paper', border: '1px solid', borderColor: 'divider', '&:hover': { bgcolor: 'action.hover' } }}
      ...
      <DatePickers.LocalizationProvider dateAdapter={AdapterDateFns}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, flexWrap: 'wrap' }}>
          <DatePickers.DateTimePicker
            ...
            slotProps={{ textField: { size: 'small', sx: { minWidth: 220, bgcolor: 'background.paper' } } }}
          />
          <DatePickers.DateTimePicker
            value={dateTo}
          ...
          />
          {hasDateFilter && (
            <Button variant="text" size="small" onClick={clearDateRange} startIcon={<X size={16} />}
          ...
          <ListingTable>
            <ListingTable.Head>
              <ListingTable.Row>
                ...
                <ListingTable.Cell sx={{ fontWeight: 600, width: 200 }}><Typography variant="caption">User</Typography></ListingTable.Cell>
                <ListingTable.Cell sx={{ fontWeight: 600, width: 160 }}><Typography variant="caption">Action</Typography></ListingTable.Cell>
                <ListingTable.Cell sx={{ fontWeight: 600, width: 100 }}><Typography variant="caption">Status</Typography></ListingTable.Cell>
                <ListingTable.Cell sx={{ fontWeight: 600 }}><Typography variant="caption">Details</Typography></ListingTable.Cell>
              ...
              {filteredLogs.map((entry) => {
                ...
                return (
                  <ListingTable.Row key={entry.id}>
                    ...
                    <ListingTable.Cell>
                      <Typography sx={{ fontSize: 13, color: 'text.secondary' }}>{entry.user}</Typography>
                    </ListingTable.Cell>
                    <ListingTable.Cell>
                      <Chip
                        label={entry.action?.replace(/_/g, ' ')}
                    ...
                    <ListingTable.Cell>
                      {entry.outcome === 'success' ? (
                        <Chip icon={<CheckCircle size={16} />} label="Success" size="small"
                          sx={{ bgcolor: '#dcfce7', color: '#166534', fontWeight: 600, fontSize: '11px' }} />
                      ) : (
                        <Chip icon={<XCircle size={16} />} label="Failed" size="small"
                          sx={{ bgcolor: '#fee2e2', color: '#991b1b', fontWeight: 600, fontSize: '11px' }} />
                      )}
                    </ListingTable.Cell>
                    <ListingTable.Cell>
                      <Typography sx={{ fontSize: 13, color: 'text.secondary' }}>{entry.details}</Typography>
                      {entry.reason && (
                        <Typography sx={{ fontSize: 12, color: entry.outcome === 'success' ? 'success.main' : 'error.main', mt: 0.5 }}>
                          {entry.reason}
                        </Typography>
                      )}
                    </ListingTable.Cell>
                    <ListingTable.Cell>
                      <IconButton size="small" onClick={() => setSelectedEntry(entry)} title="View details"
      ...
      {!loading && filteredLogs.length > 0 && hasMore && (
        <Box sx={{ display: 'flex', justifyContent: 'center', mt: 1 }}>
          <Button
            ...
            startIcon={loadingMore ? <CircularProgress size={18} color="inherit" /> : <ChevronDown size={18} />}
            sx={{ minWidth: 200, borderColor: 'divider', color: 'text.primary', '&:hover': { borderColor: 'primary.main', bgcolor: 'action.hover' } }}
          >
